...
class Loop(object):
    def __init__(self, first_node, body_nodes, graph):
        ...
        self.start_nodes = []   # The start nodes of the loop.
        self.end_nodes = []     # The end nodes of the loop.
        self.in_nodes = []      # The in nodes of the loop.
        self.out_nodes = []     # The out nodes of the loop.
...
class LoopFinder(object):
    ...
    def _get_original_node(self, cfg, node):
        if hasattr(node, 'node_type') and (node.node_type == 'Call' or node.node_type == 'iCall' and node.target != 'iCall'):
    ...
    def _initial_loop_object(self, func_cfg, loop_graph):
        ...
        for node in loop_graph.nodes():
            # print("Loop :%s %s" % (node, type(node)))
            original_node = self._get_original_node(func_cfg, node)
            # print("Org-node: %s" % (original_node))
        ...
        if len(start_nodes) == 0:
            ...
            logger.info("The loop %s has not start nodes. Do it future." % (loop))
