...
struct_similar_hash_values = [9241, 9337, 9343, 9371, 9397, 9403, 9419, 9433, 9511, 9533, 9547, 9613]
MAX_LEVLE = len(struct_similar_hash_values)
special_values = {"zero": 9851, "embedded": 9859, "array": 9871, "reg": 9883, "sym": 9887, "mul": 43177}
...
def only_contain_argument(ast, arguments=None):
    """
    Whether ast only contain arguments?
    """
    if ast is None:
        return False
    ...
    for leaf_ast in ast.recursive_leaf_asts:
        if (leaf_ast.op == 'BVS' ):
            var = leaf_ast.args[0]
            if not is_arg_symbol(var, arguments):
                return False

        elif (leaf_ast.op == 'BVV'):
            value = leaf_ast.args[0]
            if get_scope(value) == 'stack':
                return False
...
def get_offset_hash_value(ast):
    """
    Get the strcut hash value of the offset ast.
    Case: ast = R0 or Store/Load(ptr) + off or Load/Store(ptr)
    or R0 + Load(xxx)) or R0 + i * s
    """
    op = ast.op
    if op in atoms_ops or op in ls_ops:
        return special_values['zero']
    elif op in offset_ops:
        args = ast.args
        if len(args) == 2:
            opnd1 = args[1]
            if opnd1.op == 'BVV':
                ...
                return special_values["reg"]
            elif opnd1.op == '__mul__':
                return special_values["array"]
            else:
                return special_values["sym"]
        else:
            print("Do-it-future: %s" % (ast))
            return special_values["sym"]
    else:
        print("Do-it-future: %s" % (ast))
        return special_values["sym"]

def get_base(ast, trace_sims):
    """
    TODO!!! Get the base.
    """
    op = ast.op
    if op in atoms_ops or op in ls_ops:
        return ast
    elif op not in offset_ops:
        raise Exception("We couldn't find base in %s" % (ast))
    ...
    while stack_list:
        ...
        for arg in op_data.args:
            ...
            elif arg.op in ls_ops:
                unknows.append(arg)

    if len(bases) == 0 and len(unknows) >= 1:
        bases.append(unknows[0])
...
def calculate_ast_struct_similar_id(ast, trace_sims):
    """
    Calculate the AST's similar struct ID. Only consider the struct level and 'base+offset'.
    """
    leaf_info = []

    r = collect_sse_info(ast, 0, leaf_info, trace_sims)
    # print(leaf_info)
    if r == -1 or len(leaf_info) < 3:
        return 0
...
def collect_sse_info(ast, struct_level, leaf_info, trace_sims):
    if struct_level >= MAX_LEVLE:
        return -1
    level_con = struct_similar_hash_values[struct_level]
    # print("%s struct levle: %d %d" % (ast, struct_level, level_con))
    ...
    if ast.op in ['Load', 'Store']:
        ...
        addr = ast.args[0]
        base = get_base(addr, trace_sims)
        # print("--> %s with-base %s" % (addr, base))
        if base is None:
            print("Get-base-error: %s %s" % (addr, base))
            return -1
        off = get_offset_hash_value(addr)
        info = (off, level_con, struct_level)
        leaf_info.append(info)
        # print("Collect-info: %s" % (str(info)))

        if base.op != 'Load' and base.op != 'Store' and base.op not in atoms_ops:
            print("Get-base-error: %s %s" % (addr, base))
            return -1
        collect_sse_info(base, struct_level, leaf_info, trace_sims)
    elif ast.op in atoms_ops:
        return 1
...
def extract_simple_base_offset(addr):
    """
    ...
    if addr.op == 'BVS':
        ...
        if arg_0.op in base_ops and arg_1.op == 'BVV':
            base_offset = (arg_0, arg_1.args[0])
...
def find_binop_ptr_v1(addr_var, trace_ast, trace_sims):
    """
    In forward, find the alias with binop pointer.
    """
    find_flag = False
...
def calculate_sse_struct_id(ast, level=0):
    """
    Calculate the ID of the SSE.
    :param ast: A abstract syntax tree from Claripy.
    :param level: similarity level
    """
    if level == 3:
        return calculate_sse_struct_id_levelThree(ast)
    elif level == 4:
        return calculate_sse_struct_id_levelFour(ast)
    elif level == 5:
        return calculate_sse_struct_id_levelFive(ast)
    else:
        return -1

def calculate_sse_struct_id_levelFour(ast):
    """
    Only the 'base + offset' with two level are considered to calculate the ID of the SSE.
    :param ast: A abstract syntax tree from Claripy.
    """
    # print("@@DEUBG--> Cal-structID-level-Four: %s" % (ast))
    base_offset_values = []
    mul_values = set()
    base = ast
    while True:
        off_asts, off_info = [], {}
        base = parse_base_offset(base, off_asts, off_info)
        if base is None:
            # print("##DEUBG--> (No Base) should igonre this %s" % (ast))
            return -1
        # print("base: %s" % (base))
        # print("off_asts: ", off_asts)
        # print("off_info: ", off_info)

        base, base_id = cal_base_id(base, level=3)
        off_value = obtain_offset(off_asts, off_info)
        if off_value == 0xFFFFFFFF:
            return -1
        base_offset_values.append((base_id, off_value))
        # print("Get-offset-value: %d" % (off_value))

        if 'mul' in off_info:
            mul_values |= off_info['mul']

        if base.op in atoms_ops:
            break
        base = base.args[0]
    # print("Base-Offset-> ", base_offset_values)
    if len(base_offset_values) == 0:
        return -1
    struct_id = 1
    i = 0
    for v in base_offset_values[::-1]:
        struct_id = struct_id * hash(v)
        i += 1
        if i >= 2:
            break
    for mul_value in mul_values:
        struct_id = struct_id * mul_value * special_values['mul']
    # print("Sturct-ID: %s %d" % (ast, struct_id))
    return struct_id

def calculate_sse_struct_id_levelThree(ast):
    """
    Only the base address and offset are considered to calculate the ID of the SSE.
    :param ast: A abstract syntax tree from Claripy.
    """
    offset_values = set()
    mul_values = set()
    ...
    while True:
        ...
        # cal_offset_id(off_asts, off_info, level=3)
        off_value = obtain_offset(off_asts, off_info)
        offset_values.add(off_value)
        # print("Get-offset-value: %d" % (off_value))
        ...
        if base.op in atoms_ops:
            break
        base = base.args[0]
    # print(base_ids, offset_values, mul_values)
    struct_id = 1
    for base_id in base_ids:
        struct_id = struct_id * base_id
    for off_value in offset_values:
        struct_id = struct_id * off_value if off_value else struct_id * 33967
    for mul_value in mul_values:
        struct_id = struct_id * mul_value * special_values['mul']
    print("Sturct-ID: %s %d" % (ast, struct_id))
    return struct_id

def calculate_sse_struct_id_levelFive(ast):
    """
    Only the 'base + offset' with one level are considered to calculate the ID of the SSE.
    :param ast: A abstract syntax tree from Claripy.
    """
    print("@@DEUBG--> Cal-structID-level-Four: %s" % (ast))
    base_offset_values = []
...
def cal_immediate_offset(off_asts):
    """
    Calculate the immediate of offset according to the off_asts.
    :param off_asts: a list which save multiple offset asts (0 means 0 offset)
    """
    off_value = 0
    flag = 0
    for op, off in off_asts:
        # print(op, off)
        if 'add' in op and off.op == 'BVV':
            off_value = off_value + off.args[0]
            flag = 1
        elif 'sub' in op and off.op == 'BVV':
            off_value = off_value - off.args[0]
            flag = 1
    off_value = 0xFFFFFFFF if flag == 0 else off_value
    return off_value

def obtain_offset(off_asts, off_info):
    """
    Obtain the offset value according to the offset information.
    :param off_asts: a list which save multiple offset asts (0 means 0 offset)
    :param off_info: a dict which save offset info (mul, load/store, ...)
    """
    if len(off_asts) == 1 and off_asts[0] == 0:
        return 0
    if ('odd' in off_info or '*' in off_info or 'op' in off_info):
        return 0xFFFFFFFF
    off_value = cal_immediate_offset(off_asts)
    return off_value

def cal_offset_id(off_asts, off_info, level=0):
    """
    Calculate the struct_id of the offset.
    :param off_asts: a list which save multiple offset asts (0 means 0 offset)
    :param off_info: a dict which save offset info (mul, load/store, ...)
    :param level: select which level to calculate id
    E.g., off_asts:  [('__add__', <BV32 i * 0x10>), ('__add__', <BV32 0x4>), ('__add__', <BV32 0x4>)]
    """

    offset_id = 43177 # Default offset id.
    if len(off_asts) == 1 and off_asts[0] == 0:
        return 33967 # offset is zero, its default id.
    if level == 3:
        if ('odd' in off_info or '*' in off_info or 'op' in off_info):
            return 34273
        if 'mul' in off_info:
            mul_offsets = off_info['mul']
            for off in mul_offsets:
                offset_id += off * special_values['mul']
        # cal_immediate_offset(off_asts)
    return offset_id

def cal_base_id(base, level=0):
    """
    Calculate the struct_id of the base.
    :param base: a ast (the operator is not 'add' and 'sub')
    :param level: select which level to calculate id
    """
    def get_base(ast):
        # print(ast, ast.op, ast.args)
        if 'Ext' in ast.op:
            opnd0 = ast.args[1]
        else:
            opnd0 = ast.args[0]
        if isinstance(opnd0, (int, str)):
            raise Exception("TODO in cal_base_id %s" % (ast))
        elif opnd0.op in base_ops:
            return opnd0
        else:
            return get_base(opnd0)
    base_id = 1
    op = base.op
    ...
    if base.op == 'BVV':
        ...
        base_id = hash(base.args[0])
    elif level == 3:
        base_id = 1
    else: # TODO
        base_id = 1
    # print("Cal-base-ID: base %s, base-id %s" % (base, base_id))
    return base, base_id

# <BV32 Store(Store(Store(Load(r8 + 0x1c)) + 0x4) + i * 0x10 + 0x4 + 0x4)>: <BV32 r12>
def parse_binop(ast, off_asts, off_info):
    """
    The ast is a binop data with op +, -, *, /, <<, >>, ...
    :param ast: A abstract syntax tree (ast) from Claripy.
    """
    # print("parse-binop: %s" % (ast))
    base = None
    for i, opnd in enumerate(ast.args):
        # print('opnd %s' % (opnd))
        if i == 0:
            if ('add' not in opnd.op and 'sub' not in opnd.op):
                base = opnd
            else:
                base = parse_binop(opnd, off_asts, off_info)
        else:
            off_asts.append((ast.op, opnd))
            if opnd.op == 'BVV':
                opnd_value = opnd.args[0]
                if opnd_value %2 != 0: # maybe ignore this offset ?
                    off_info['odd'] = 1
            elif 'mul' in opnd.op: # maybe 'i * 0x10'
                if len(opnd.args) == 2:
                    arg1, arg2 = opnd.args
                    if arg2.op == 'BVV' and arg1.op == 'BVS':
                        if 'mul' not in off_info:
                            off_info['mul'] = set()
                        off_info['mul'].add(arg2.args[0])
                    else:
                        off_info['*'] = 1
                else:
                    raise Exception("TODO in parse_binop %s" % (ast))
            else:
                if 'op' not in off_info:
                    off_info['op'] = set()
                off_info['op'].add(opnd.op)

    return base


def parse_base_offset(ast, off_asts, off_info):
    """
    Assume that the ast are all 'base + offset' mode.
    :param ast: A abstract syntax tree from Claripy.
    """
    # print("parse-base-offset: %s" % (ast))
    op = ast.op
    if op == 'BVS' or op == 'BVV':
        base = ast
        off_asts.append(0)
        ...
        base = ast
        off_asts.append(0)
    elif 'add' not in op and 'sub' not in op:
        return None
    elif len(ast.args) >= 2:
        base = parse_binop(ast, off_asts, off_info)
    else:
        raise Exception("TODO in parse_base_offset %s" % (ast))
    return base
