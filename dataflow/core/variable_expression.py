...
class Sim(object):
    ...
    def __init__(self, live=True, index=None, def_loc=None, var_type=None, tag=0):
        ...
        # The tag is used to mark the properties of a sim variable,
        # including - 0x1: index, ...
        self.tag = tag
...
class SimAction(object):
    ...
    def __init__(self, name=None, binop=None, action_data=None, var_type=None, live=True):
        ...
        self.store_update_loc = None
        self.load_update_loc = None
    ...
    def copy(self):
        ...
        new_sim_action.store_update_loc = self.store_update_loc
        new_sim_action.load_update_loc = self.load_update_loc
    ...
    def load_use(self, base, offset, loc, binop=None, var_type=None):
        ...
        # print(self.action_data, self.live)
        # print(loc, self.def_locs, self.name)
    ...
    def match_concrete_offset(self, op, offset, loc):
        """
        Whether sim_action and load/store ins have the same concrete offset.
        """
        # print(self.binop, self.action_data, self.action_data.args[0])
        # if (not self.live):
        #     return False
        if (loc in self.def_locs):
            return False
        if (self.action_data.op == 'Store' and not self.live):
            return False
        addr = self.action_data.args[0]
        if (len(addr.args) < 2 or addr.concrete or not ('sub' in addr.op or 'add' in addr.op)):
            return False
        addr_op, addr_offset = addr.op, addr.args[1]
        if (addr_offset.symbolic or addr_offset.args[0] != offset):
            return False
        if (('sub' in addr_op and op != '-') or ('add' in addr_op and op != '+')):
            return False
        return True

    def match_array_index_base(self, array_base):
        """
        Whether sim_action and array_base contain the same reg name.
        :param array_base: a reg name (e.g., r56)
        """
        addr = self.action_data.args[0]
        addr = addr.args[0]
        if (not ('add' in addr.op and len(addr.args) == 2)):
            return False
        opnd_1, opnd_2 = addr.args
        if (opnd_1.op == 'BVS' and opnd_1.args[0] == array_base
                and opnd_2.op == 'BVS' and 'o' in opnd_2.args[0]):
            return True
        return False
...
class TraceExpr(object):
    ...
    def deep_copy(self):
        ...
        new_trace_expr.store_loc = self.store_loc
        new_trace_expr.store_locs = self.store_locs.copy()
        new_trace_expr.load_locs = self.load_locs.copy()
    ...
    def copy(self):
        ...
        new_trace_expr.inter_funcs = self.inter_funcs.copy()
        new_trace_expr.store_loc = self.store_loc
        new_trace_expr.store_locs = self.store_locs
        new_trace_expr.load_locs = self.load_locs
        return new_trace_expr
    ...
    def set_tag(self, ast_data):
        """
        :param ast_data: a ast variable in self.expr.ast
        """
        # print("-->set-tag: %s %s %s" % (self, ast_data, ast_data.op))
        op = ast_data.op
        if op == '__mul__':
            for var in ast_data.variables:
                if var in self.expr.sims:
                    self.expr.sims[var].tag = 0x1
                    # print("-->set-index-tag: %s" % (var))
    ...
    def update_store_locs(self, store_loc_info):
        """
        """
        for i, loc in store_loc_info.items():
            self.store_locs[i] = loc

    def update_taint_sources(self, other_expr):
        """
        Merget the taint sources between self expr and other expr.
        """
        if not self.expr.taint_source:
            self.expr.taint_source = other_expr.expr.taint_source
        if other_expr.expr.taint_source:
            self.expr.taint_sources.add(other_expr.expr.taint_source)
        if other_expr.expr.taint_sources:
            self.expr.taint_sources |= other_expr.expr.taint_sources
    ...
    def replace_with_recurisve_expr(self, subvariable, recurisve_expr, var_type=None):
        ...
        new_recur_expr.store_loc = self.store_loc
        new_recur_expr.store_locs = self.store_locs.copy()
        new_recur_expr.load_locs = self.load_locs.copy()
    ...
    def is_value_equal(self, other):
        return self.expr.is_value_equal(other.expr)
    ...
    def is_recursive_structure(self, wr_tmp, ld_offset):
        """
        :return: (base, offset, pos)
        """
        # print("\n@->is_recursive_structure:")
        sim_actions = self.expr.sim_actions

        for index, sim_action in sim_actions.items():
            # print(index, sim_action)
            binop, info = extract_simple_base_offset(sim_action.action_data.args[0])
            if (info is None):
                continue
            base, offset = info
            if ld_offset == offset and base.op == 'BVS' and base.args[0] == wr_tmp:
                # print("Success")
                return True, base
        return False, None

    def copy_to_recursive(self, base, offset, flag=0, recursive_type=""):
        ...
        new_recursive_expr.store_loc = self.store_loc
        new_recursive_expr.store_locs = self.store_locs.copy()
        new_recursive_expr.load_locs = self.load_locs.copy()
        ...
        new_recursive_expr.base = base
        new_recursive_expr.offset = offset
        new_recursive_expr.position = new_recursive_expr.get_recursive_base_positon
        new_recursive_expr.recursive_type = recursive_type
        return new_recursive_expr

    def infer_recursive_base_info(self, recursive_offset):
        """
        """
        sim_actions = self.expr.sim_actions
        ...
        for index, sim_action in sim_actions.items():
            binop, info = extract_simple_base_offset(sim_action.action_data.args[0])
            print(index, sim_action, info, sim_action.var_type)
            if (info is None or sim_action.var_type != 'ptr'):
                continue
            base, offset = info
            if recursive_offset == offset:
                if base.op == 'BVS' and base.args[0] == self.expr.base_ptr:
                    return True, base
                elif base.op == 'Load':
                    _, ld_info = extract_simple_base_offset(base.args[0])
                    if ld_info is None or ld_info[1] != recursive_offset:
                        return True, base
        return False, None

    def is_contain_recursive_base(self, recursive_base):
        """
        Check whether this trace_expr contain recursive_base
        :return: bool
        """
        if recursive_base.op == 'Load':
            sim_actions = self.expr.sim_actions
            for index, sim_action in sim_actions.items():
                if sim_action.action_data.__hash__() == recursive_base.__hash__():
                    return True
        elif recursive_base.op in atoms_ops:
            if recursive_base.args[0] == self.expr.base_ptr:
                return True

        return False

    def is_contain_array_index(self, ast):
        """
        Check whether this ast contain array index (e.g., 'o')
        """
        if 'o' in ast.variables:
            return True
        return False

    def get_array_base_index(self, ast):
        """
        Get the array's base and index (e.g., rax + o) in the ast
        """
        if 'add' not in ast.op:
            return None, None
        # print("@-->get_array_base_index: ", ast.args, self.expr.base_ptr)
        if len(ast.args) == 2:
            opnd0, opnd1 = ast.args[0], ast.args[1]
            if opnd0.op in atoms_ops:
                if (opnd0.args[0] == self.expr.base_ptr
                    and opnd1.op == 'BVS'
                    and opnd1.args[0] == 'o'):
                    return opnd0, opnd1
            elif 'add' in opnd0.op:
                opnd0, opnd1 = opnd0.args[0], opnd0.args[1]
                if (opnd0.op in atoms_ops
                    and opnd0.args[0] == self.expr.base_ptr
                    and opnd1.op == 'BVS'
                    and opnd1.args[0] == 'o'):
                    return opnd0, opnd1
...
class VarExpr(object):
    ...
    def __init__(self, ast, value=None, pattern=None, trace_dir=None, data_type=None, var_type=None, base_ptr=None):
        ...
        # Record the variable update path (TODO).
        self.store_location = []
        ...
        self.taint_live = 0 # Where is the tianted data live (TODO).
    ...
    def argument_or_global(self, arguments=None):
        ...
        for var, sim in self.sims.items():
            if type(var) is str and 'o' not in var and not is_arg_symbol(var, arguments):
                return False
    ...
    def replace(self, subvariable, replacement, re_sim_actions=None, sub_type=None, rep_type=None, rep_info=None, base_ptr=None):
        ...
        new_expr.update_sim_tags(subvariable, replacement, self.sims)
    ...
    def update_sim_tags(self, subvariable, replacement, old_sims):
        """
        Update sim's tag in new-expr with the old tag.
        """
        if subvariable.op != 'BVS':
            return
        subvar = subvariable.args[0]
        if subvar not in old_sims:
            return
        subvar_tag = old_sims[subvar].tag
        for repvar in replacement.variables:
            if repvar in self.sims:
                self.sims[repvar].tag = subvar_tag
...
class RecursiveExpr(TraceExpr):
    def __init__(self, expr, index=None, base=None, offset=None, position=None):
        ...
        # The recursive type include ['Inc', 'Chain']
        self.recursive_type = ""
    ...
    def __repr__(self):
        if self.expr.value is None:
            return '<R-Expr[%s] %s (%s) (%s) (%s-%s)>' % (self.recursive_type, self.expr.ast, self.expr.base_ptr, self.expr.var_type, self.expr.trace_dir, self.index)
        else:
            if isinstance(self.expr.value, int):
                return '<R-Expr[%s] %s: 0x%x (%s) (%s) (%s-%s)>' % (self.recursive_type, self.expr.ast, self.expr.value, self.expr.base_ptr, self.expr.var_type,  self.expr.trace_dir, self.index)
            else:
                return '<R-Expr[%s] %s: %s (%s) (%s) (%s-%s)>' % (self.recursive_type, self.expr.ast, self.expr.value, self.expr.base_ptr, self.expr.var_type, self.expr.trace_dir, self.index)
    ...
    def deep_copy(self):
        ...
        new_recursive_expr.recursive_type = self.recursive_type
        new_recursive_expr.inc_records = self.inc_records[:]
        return new_recursive_expr
    ...
    def make_backward_copy(self):
        ...
        new_trace_expr.recursive_type = self.recursive_type
        new_trace_expr.forward_path = self.forward_path.copy()
        new_trace_expr.backward_path = self.backward_path.copy()
        new_trace_expr.cycle_locs = self.cycle_locs
        new_trace_expr.guard = self.guard
