...
def infer_sim_action_type(ast, sims, sim_action_info, var_type):
    ...
    if ast.op == '__add__':
        ...
        op, opnds_type = '-', []
        if len(ast.args) == 2:
            ty = get_child_ast_type(ast, sims, sim_action_info)
...
def get_sim_type(block, var):
    var_type = None
    if type(var) is str and 'r' in var:
...
def infer_variable_type(cfg, block, opnds_info, var_type):
    ...
    if op == '+':
        ...
        if var_type == 'ptr':
            ...
            if opnd2_type and opnd2_type != 'ptr' and var_type and var_type != 'ptr':
                update_variable_type(block, opnd1, var_type)
...
def update_tmp_alias_var_type(block):
    live_defs = block.live_defs
    for tmp, reg_alias in block.tmp_alias.items():
        tmp_at = live_defs[tmp]
        for reg in reg_alias:
            if reg in live_defs:
                ret_at = live_defs[reg]
                ret_at.var_type = tmp_at.var_type
                # print(" xxx %s\n %s" % (tmp_at, ret_at))
...
def update_action_var_type(cfg, action):
    ...
    # print("Update-var-type -%s %s" % (block, action))
    ...
    update_tmp_alias_var_type(block)
