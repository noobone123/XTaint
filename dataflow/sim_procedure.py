...
class SimProcedure:
    ...
    def execute(self, block, project, run_funciton, flow_dir=None, arguments=None, caller_function=None, ret_to=None, purpose=0):
        ...
        # print("xxxx-> %s %s" % (self.arguments_info, inst.arguments_info))
    ...
    def get_value(self, reg):
        # if self.block.node_type in ['Call', 'iCall', 'Extern']:
        #     callsite = list(self.block.predecessors)[0]
    ...
    def get_string_len(self, s):
        ...
        for i in range(max_str_len):
            c = self.state.memory.load(s+i, 1)
            conc_char = self.state.solver.eval_one(c)
            if conc_char == 0:
                result = i
                break

        result = self.state.solver.BVV(result, s.size()) if result is not None else self.state.solver.BVS('s', s.size())
        return result
    ...
    def propagate_int_taint_to_ret(self, ptr, name=None, cons_type=0):
        ...
        if cons_type == 4:
            for new_expr in new_exprs:
                taint_src = new_expr.expr.taint_source
                self.collect_value_constraint(taint_src)
    ...
    def collect_value_constraint(self, taint_src):
        if taint_src not in parameter_constraints:
            i = (0, None)
            src_hex = '%x' % (taint_src)
            parameter_constraints[src_hex] = [i]

    def collect_string_constraint(self, cmp_arg, taint_srcs, cmp_flag):

        str_ptr = get_value(self.block, cmp_arg)
        if str_ptr is None:
            return
        con_str = self.get_constant_string(str_ptr, 128, False)
        # print(con_str, taint_srcs, cmp_flag)
        if not con_str:
            return
        for ts in taint_srcs:
            addr = '%x' % ts
            if addr not in parameter_constraints:
                parameter_constraints[addr] = []
            i = (cmp_flag, con_str)
            if i not in parameter_constraints[addr]:
                parameter_constraints[addr].append(i)

    def add_special_constraint_v2(self, s1, s2, cons_type, name=None, cmp_flag=None):
        """
        Some libc function will attach constraint to taint data.
        ...
        for trace_expr in self.block.forward_exprs:
            ...
            if trace_ast.op == 'BVS':
                if s1 in sims:
                    ptr_id = trace_expr.expr.ptr_id
                    ptr_ids.add(ptr_id)
                    taint_srcs.add(trace_expr.expr.taint_source)
                    s1_flag = 1
                elif s2 in sims:
                    ptr_id = trace_expr.expr.ptr_id
                    ptr_ids.add(ptr_id)
        ...
        if s1_flag:
            self.collect_string_constraint(s2, taint_srcs, cmp_flag)
        elif s2_flag:
            self.collect_string_constraint(s1, taint_srcs, cmp_flag)
        if cmp_flag == 2:
            return
    ...
    def get_string(self, ptr):
        ...
        :param ptr: a pointer
        """
        str_ptr = get_value(self.block, ptr)
        if str_ptr is None:
    ...
    def get_constant_string(self, addr, max_str_len=512, special_char=True):
        """
        Get constant string from memory.
        :param addr: a concret address which may contain a string.
        :max_str_len: the max length of a constant string, its default value is 512 bytes.
        :special_char: whether it contains non-printable characters.
        """
        con_str = ''
        ...
        ptr_ast = BVV(addr)
        for i in range(max_str_len):
            c = self.state.memory.load(ptr_ast+i, 1)
            conc_char = self.state.solver.eval_one(c)
            if conc_char == 0:
                break
            elif not special_char and (conc_char <= 0x20 or conc_char > 0x7E):
                return ''
            con_str += chr(conc_char)
        # print(con_str)
        return con_str
    ...
    def check_alias(self, arg1, arg2, alias_type):
        """
        Check whether arg1 and arg2 are alias.
        :param alias_type: the alias type include 'must', 'may', 'not'.
        """
        for trace_expr in self.block.forward_exprs:
            print("@@:1378-->ckeck_alias: %s %s %s %s" % (trace_expr, arg1, arg2, alias_type))
            sims = trace_expr.expr.sims
            trace_ast = trace_expr.expr.ast

            if arg2 in sims and trace_ast.op == 'BVS':
                print("Found-%s-alias %x %x %s" % (alias_type, trace_expr.expr.taint_source, self.block.addr, trace_expr.expr.taint_sources))
                if (alias_type == 'must' or alias_type == 'may') and self.block.addr in trace_expr.expr.taint_sources:
                    print("Good-find-alias: %s %s %x" % (arg1, arg2, self.block.addr))
                    # print("org-expr: %s %s" % (trace_expr, trace_expr.expr.taint_source))
                    ALIAS_SOURCES[self.block.addr] = 1
                elif alias_type == 'not' and self.block.addr in trace_expr.expr.taint_sources:
                    ALIAS_SOURCES[self.block.addr] = -1


from global_config import default_arguments, basic_types, default_arg_names
from dataflow.core import Action, CodeLocation
from dataflow.core.parse_ast import BVV, BVS, find_ptr_taint_v1, get_value, get_scope, get_mem_permission, not_contain_ls, is_str_pointer
from dataflow.core.variable_type import update_action_var_type
from dataflow.core.data_collector import weaks_only_length, parameter_constraints
from dataflow.core.data_process import ALIAS_SOURCES
from dataflow.core.variable_expression import construct_trace_expr
